...
1. **Reflash the ZS3L** to firmware that can send commands — e.g. a Zigbee stack where it acts as a secondary coordinator or issues bound commands. Highest payoff, highest cost: you need the toolchain for that 802.15.4 part, a flashing path (likely internal SWD/UART), and you risk it dropping out of the existing mesh / needing re-pairing. This is the only path that's truly WiFi- and HA-independent end to end.
